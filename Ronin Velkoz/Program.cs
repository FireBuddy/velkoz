...

            {
                ...
                foreach (var perpendicular in Perpendiculars)
                {
                    if (Handle != null)
                    {
                        ...
                        var collisionObjects = EntityManager.Heroes.Enemies.Where(it => it.IsValidTarget(1000));
                        ...
                        {
	                if ( Prediction.Position.Collision.LinearMissileCollision(hero, startPos, endPos + 300, 2000, 100, 0))
                    	{
